...
describe('Whale Tracker System 🐋', () => {
    ...
    describe('Data Retrieval', () => {
        ...
        it('should get whale alerts for user', async () => {
            ...
            mockPrisma.whaleFollower.findMany.mockResolvedValue([
                { whaleAddress: 'whale1' }
            ]);

            // 2. Get whale IDs
            mockPrisma.whaleWallet.findMany.mockResolvedValue([
                { id: 'id1', walletAddress: 'whale1' }
            ]);

            // 3. Get alerts
            mockPrisma.whaleAlert.findMany.mockResolvedValue([
                { id: 'alert1', description: 'Big Buy' }
            ]);
