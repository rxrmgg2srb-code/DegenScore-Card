...
export default function DegenCard() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (pendingProfileData) {
        logger.info('📝 Saving profile to database after payment:', { walletAddress });
        ...
        logger.info('🎨 Regenerating premium card with profile data...');
        const imageResponse = await fetch('/api/generate-card', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      logger.error('Error updating profile after payment', error instanceof Error ? error : undefined, {
        error: String(error),
      });
  ...
  const handleProfileSubmit = async (profileData: ProfileData) => {
    ...
    logger.info('📝 Profile data saved temporarily:', profileData);
