...
function analyzeCoverage() {
    ...
    if (overallAvg >= COVERAGE_THRESHOLD) {
        console.log(`✅ GOAL ACHIEVED! Coverage is ${overallAvg.toFixed(2)}% (>= ${COVERAGE_THRESHOLD}%)`);
    } else {
        console.log(`⚠️  Need ${(COVERAGE_THRESHOLD - overallAvg).toFixed(2)}% more coverage to reach ${COVERAGE_THRESHOLD}%`);
    }
